...
export class OpmlService {
  ...
  async exportOPML(userId: number, batchSize = 10) {
    const user = await this.knexService.connection<User>('users').where({ id: userId }).first();
    ...
    let offset = 0;
    let categories = await this.knexService
      .connection<Category>('categories')
    ...
    while (categories.length > 0) {
      for (const category of categories) {
        let offset2 = 0;
        let feeds = await this.knexService
      ...
      offset += batchSize;
      categories = await this.knexService
        .connection<Category>('categories')
        .where({ user_id: userId })
        .offset(offset)
        .limit(batchSize)
